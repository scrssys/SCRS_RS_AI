...
from ulitities.base_functions import load_label, load_img_by_gdal
...
def CRFs(original_image_path, predicted_image_path, CRF_image_path):
    ...
    anno_binary = cv2.imread(predicted_image_path, cv2.IMREAD_GRAYSCALE)
    # anno_binary = load_label(predicted_image_path)
    colors,labels = np.unique(anno_binary, return_inverse=True)
